...
pub fn run_app() {
    // Reap the bundled server if the app is SIGTERM'd (logout/shutdown/kill),
    // which bypasses Tauri's normal exit path. Must precede Builder::run().
    #[cfg(unix)]
    ...
    app_builder
        ...
        .setup(move |app| {
            ...
            // Start the bundled DeepSeek Harness server and point the window at
            // its dynamically allocated port (`--port 0`), so the desktop app
            // never collides with a browser instance already on 3080.
            app.manage(sidecar::ServerProcess::new());
            let mut effective_config = pake_config.clone();
            match sidecar::start(app.app_handle()) {
                Some((child, url)) => {
                    if let Some(state) = app.try_state::<sidecar::ServerProcess>() {
                        if let Ok(mut guard) = state.0.lock() {
                            *guard = Some(child);
                        }
                    }
                    effective_config.windows[0].url = url;
                }
                None => {
                    eprintln!("[dsh] server failed to start; window will show an error page");
                    // Point at an unroutable port so the webview renders a
                    // visible connection error instead of a blank window.
                    effective_config.windows[0].url = "http://127.0.0.1:1".to_string();
                }
            }

            let window = match set_window(app.app_handle(), &effective_config, &tauri_config) {
                Ok(window) => window,
                Err(err) => {
                    // Window creation failed (e.g. unwritable app-data dir):
                    // make sure the spawned server is not left orphaned.
                    if let Some(state) = app.try_state::<sidecar::ServerProcess>() {
                        state.kill();
                    }
        ...
        .run(move |app, event| {
            // Kill the bundled server when the app exits (window closed or
            // Cmd+Q), so no orphan node process is left behind.
